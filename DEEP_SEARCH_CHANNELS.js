...
async function deepSearch() {
    console.log("Deep searching for channel data (3090 subscribers or 414 videos)...");
    ...
    for (const dbPath of allDbPaths) {
        ...
        await new Promise((resolve) => {
            db.all("SELECT name FROM sqlite_master WHERE type='table' AND name='youtube_channels'", (err, tables) => {
                ...
                db.all("SELECT * FROM youtube_channels WHERE subscriber_count LIKE '%3090%' OR video_count LIKE '%414%'", (err, rows) => {
