...
class YoutubeChannel {
  ...
  static findBySlug(userId, slug) {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT * FROM youtube_channels WHERE user_id = ? AND slug = ?',
        [userId, slug],
        (err, row) => {
          if (err) return reject(err);
          resolve(row);
  ...
  static async create(data) {
    ...
    return new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO youtube_channels (id, user_id, channel_id, channel_name, channel_thumbnail, subscriber_count, video_count, access_token, refresh_token, is_default, slug, description, channel_color)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          data.user_id,
