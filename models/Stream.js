...
class Stream {
  ...
  static findAll(userId = null, filter = null, channelId = null) {
    return new Promise((resolve, reject) => {
      let query = `
        SELECT s.*, 
      ...
      if (channelId) {
        conditions.push('s.youtube_channel_id = ?');
  ...
  static findAllPaginated(userId = null, options = {}) {
    const { page = 1, limit = 10, filter = null, search = '', channelId = null } = options;
