...
class Video {
    ...
    return new Promise((resolve, reject) => {
      ...
      db.run(
        `INSERT INTO videos (
          ...
          duration, format, resolution, bitrate, fps, user_id, youtube_channel_id,
          created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id, data.title, data.filepath, data.thumbnail_path, data.file_size,
          data.duration, data.format, data.resolution, data.bitrate, data.fps, data.user_id, data.youtube_channel_id || null,
          now, now
        ],
        function (err) {
          if (err) {
  ...
  static findAll(userId = null, channelId = null) {
    return new Promise((resolve, reject) => {
      ...
      // Let's keep it flexible. If channelId passed as 'null' (explicitly), we might want global.
      // But the user said: "agar ... tidak tercampur".
      // Use strict filtering if channelId is passed.
