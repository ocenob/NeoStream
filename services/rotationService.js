...
async function checkRotations() {
  ...
  try {
    ...
    for (const rotation of activeRotations) {
      ...
      if (!activeRotationStreams.has(streamKey)) {
        if (startingStreams.has(streamKey)) continue;
...
async function startRotationStream(rotation, item) {
  try {
    ...
    const thumbnailToUpload = item.original_thumbnail_path || item.thumbnail_path || item.video_thumbnail;
    console.log('[RotationService] Thumbnail upload check:', {
      selected: thumbnailToUpload
    });

    if (thumbnailToUpload) {
      try {
        // Extract filename from path if it contains directory separators
        let thumbnailFilename = thumbnailToUpload;
        if (thumbnailToUpload.includes('/')) {
          const parts = thumbnailToUpload.split('/');
        ...
        console.log('[RotationService] Thumbnail path resolved to:', thumbnailPath);
        console.log('[RotationService] File exists:', fs.existsSync(thumbnailPath));

        if (fs.existsSync(thumbnailPath)) {
          console.log('[RotationService] Uploading thumbnail to YouTube for broadcast:', broadcast.id);
          ...
          console.log('[RotationService] Thumbnail uploaded successfully');
        } else {
          console.warn('[RotationService] Thumbnail file not found at:', thumbnailPath);
        }
      } catch (thumbError) {
        console.error('[RotationService] Error setting thumbnail:', thumbError.message);
        console.error('[RotationService] Full error:', thumbError);
      }
    } else {
      console.warn('[RotationService] No thumbnail available for upload');
    }
    ...
    // Use the duration generated by AutoScheduler (Random 3-7h) if exists
    let streamDuration = item.duration || null;

    if (item.video_id && item.video_id.startsWith('playlist:')) {
      actualVideoId = item.video_id.substring(9);

      // Only calculate if we don't have a preset duration
      if (!streamDuration) {
        try {
          const playlistDetails = await Playlist.findByIdWithVideos(actualVideoId);
          if (playlistDetails && playlistDetails.videos) {
            streamDuration = playlistDetails.videos.reduce((acc, v) => acc + (v.duration || 0), 0);
            console.log(`[RotationService] Calculated playlist duration: ${streamDuration}s`);
          }
        } catch (err) {
          console.warn('[RotationService] Failed to calculate playlist duration:', err.message);
        }
      } else {
        console.log(`[RotationService] Using assigned item duration: ${streamDuration}s`);
      }
    }

    console.log('[RotationService] Creating stream record...');
