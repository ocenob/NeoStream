...
async function buildFFmpegArgsForPlaylist(stream, playlist) {
  ...
  if (!hasAudio) {
    console.log('[StreamingService] Playlist has no audio. Injecting silent audio with transcoding.');
    // FIX: Inject silent audio and force transcoding to ensure valid RTMP output
    return [
      '-nostdin',
      '-loglevel', 'info',
      '-re',
  ...
  if (!stream.use_advanced_settings) {
    console.log('[StreamingService] Playlist with Audio: Enforcing transcoding for stability.');
    // FIX: Force transcoding even in standard mode for playlists to handle mixed codecs/files
    return [
      '-nostdin',
      '-loglevel', 'info',
      '-re',
