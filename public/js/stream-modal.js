...
// Manual Handler for Create Button (No Form Submit Event)
// DEPRECATED: Logic moved to channel_dashboard.ejs V2 submission handler
/*
document.addEventListener('click', async function (e) {
  const btn = e.target.closest('#createStreamBtn');
  if (btn) {
    console.log('[Debug] Create Stream button clicked');
    ...
    try {
      ...
      if (result.success) {
        ...
        alert('Failed to create stream: ' + (result.error || 'Unknown error'));
      }
    } catch (error) {
